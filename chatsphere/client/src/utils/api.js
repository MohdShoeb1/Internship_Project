...
export const roomAPI = {
  ...
  createGroup:  (data)           => api.post('/rooms/group', data, { headers: { 'Content-Type': 'multipart/form-data' } }),
  searchUsers:  (q)              => api.get(`/rooms/search/users?q=${encodeURIComponent(q)}`),
...
export const messageAPI = {
  ...
  sendMessage:    (roomId, data)     => api.post(`/messages/${roomId}`, data, { headers: { 'Content-Type': 'multipart/form-data' } }),
  searchMessages: (roomId, q)        => api.get(`/messages/${roomId}/search?q=${encodeURIComponent(q)}`),
