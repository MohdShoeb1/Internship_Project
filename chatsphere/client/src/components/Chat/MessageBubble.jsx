...
export default function MessageBubble({ message: msg, isMe, prevMsg, currentUser, onReact, onDelete, onEdit, onPin, onReply }) {
  ...
  const readCount = msg.readBy?.filter(id => id !== currentUser?.id).length || 0;
    ...
    <div className={`msg-group ${isMe ? 'me' : 'them'}`}>
      ...
      <div className="msg-row-inner">
        ...
        <div className="bubble-col">
          ...
          {editing ? (
            ...
            >
              ...
              {msg.type === 'image' && msg.fileUrl && (
                <img
                  src={`${process.env.REACT_APP_SOCKET_URL}${msg.fileUrl}`}
                  alt="img"
                  className="msg-image"
                  onClick={() => window.open(`${process.env.REACT_APP_SOCKET_URL}${msg.fileUrl}`, '_blank')}
                />
              )}
              {msg.type === 'file' && msg.fileUrl && (
                <a
                  href={`${process.env.REACT_APP_SOCKET_URL}${msg.fileUrl}`}
                  target="_blank"
                  rel="noreferrer"
              ...
              {msg.type === 'audio' && msg.fileUrl && (
                <audio controls src={`${process.env.REACT_APP_SOCKET_URL}${msg.fileUrl}`} className="msg-audio" />
              )}
              {msg.type === 'video' && msg.fileUrl && (
                <video controls src={`${process.env.REACT_APP_SOCKET_URL}${msg.fileUrl}`} className="msg-video" />
              )}
              {/* Text content */}
          ...
          <div className="bubble-meta">
            ...
            {isMe && (
              <span className="read-ticks" title={readCount > 0 ? 'Read' : 'Sent'}>
                {readCount > 0 ? <span style={{ color: '#9d98f8' }}>✓✓</span> : '✓'}
              </span>
            )}
          </div>
        ...
        <div className={`msg-actions ${isMe ? 'me' : 'them'}`}>
          ...
          {isMe && <button title="Delete" onClick={() => onDelete(msg.id)} className="del-btn">🗑</button>}
        </div>
      </div>
      ...
      {showMenu && (
        <div className="context-menu" ref={menuRef} onMouseLeave={() => setShowMenu(false)}>
          ...
          {isMe && <button onClick={() => { setEditing(true); setShowMenu(false); }}>Edit</button>}
